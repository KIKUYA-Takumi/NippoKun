...
urlpatterns = [
    ...
    url(r'^(\w+)/score_entries/', login_required(CreateScore.as_view()), name='create_score'),
    url(r'^(\w+)/score/(?P<pk>[0-9]+)/edition/$', login_required(UpdateScore.as_view()), name='update_score'),
    url(r'^(\w+)/score/(?P<pk>[0-9]+)/delete/$', login_required(DeleteScore.as_view()), name='delete_score'),
